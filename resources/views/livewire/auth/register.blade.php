<div class="flex justify-center items-center h-dvh w-screen">
    <form wire:submit.prevent="insert_user" class="border-2 border-[#3b83f685] px-5 pb-5 shadow-md shadow-blue-300">
        ...
        <div class="flex justify-between flex-col gap-5 max-w-[350px] md:max-w-[400px] w-[350px] md:w-[400px] text-sm">
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-5">
                <div>
                    ...
                    <div class="relative">
                        <input type="password" wire:model="password" name="password" placeholder="Enter password" min="8" class="w-full py-2 px-5 mt-1 focus:outline-none border-0 border-b-2 border-gray-300 placeholder:text-gray-400 text-gray-500">
                        <span class="absolute left-0 bottom-[9px] text-gray-400"><i class="fa-solid fa-lock"></i></span>
                    </div>    
                    @error('password') <span class="text-[10px] text-red-400">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="confirm_password" class="text-gray-700">Confirm Password</label>
                    <input type="password" name="confirm_password" wire:model="confirm_password" placeholder="Re-enter password" min="8" class="w-full py-2 mt-1 focus:outline-none border-0 border-b-2 border-gray-300 placeholder:text-gray-400 text-gray-500">
                    @error('confirm_password') <span class="text-[10px] text-red-400">{{ $message }}</span> @enderror
                </div>
            </div>
            <div>
                <button class="bg-blue-600 cursor-pointer text-white py-3 px-5 mt-5 focus:outline-none border-0 border-b-2 border-gray-300 placeholder:text-gray-400 w-full">Register</button>
                {{-- login button here if user has an account already --}}
                <div class="mt-1 text-xs">
                    <span class="text-gray-500">Already have an account?</span>
                    <a href="{{ route('login') }}" class="text-blue-600 cursor-pointer py-1 focus:outline-none border-0 border-b-2 border-blue-400 placeholder:text-blue-400 w-full">Login</a>
                </div>
            </div>
        </div>
        @if ($this->ok)
